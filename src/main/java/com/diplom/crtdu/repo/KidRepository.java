...
public interface KidRepository extends CrudRepository<Kid,Long> {
    ...
    @Query(value = "SELECT k.*\n" +
            ...
            "WHERE kk.krujok_id = :id1 and k.archive = 0\n" +
            "AND NOT EXISTS (\n" +
            "  SELECT 1\n" +
            "  FROM uchastnik u\n" +
            "  WHERE u.kid_id = k.id\n" +
    ...
    List<Kid> findByGrazhdanstvoAndArchive(String gr, Boolean archive);
    List<Kid> findBySexAndArchive(Boolean sex, Boolean archive);

    List<Kid> findByArchive(Boolean archive);
